...
export const BandRankings = () => {
  ...
  const filteredPlayers = useMemo(() => {
    return players.filter(player => `${player.first_name} ${player.last_name}`.toLowerCase().includes(searchTerm.toLowerCase()) || player.nationality.toLowerCase().includes(searchTerm.toLowerCase()));
  }, [players, searchTerm]);
  ...
  return <div className="space-y-6">
      ...
      <div className="relative max-w-md mx-auto">
        ...
        <Input placeholder="Cerca giocatori..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10" />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        return <Card key={bandNumber} className="relative overflow-hidden">
            ...
            backgroundColor: bandColor
              ...
              <CardHeader className="pb-4">
                <CardTitle className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex items-center justify-center w-10 h-10 rounded-full text-white shadow-lg" style={{
                  backgroundColor: bandColor
                }}>
              ...
              <CardContent className="space-y-3">
                {playersInBand.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                    Nessun giocatore in questa fascia
                  </div> : playersInBand.map((player, index) => <div key={player.id} className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => handlePlayerClick(player)}>
                      <div className="flex items-center gap-3">
                        <Badge variant="secondary" className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm" style={{
                  ...
                  borderColor: bandColor
                        ...
                        <Avatar className="h-10 w-10">
                          <AvatarFallback className="text-white font-semibold" style={{
                    backgroundColor: bandColor
                  }}>
      ...
      <PlayerStatsModal player={selectedPlayer} open={isStatsModalOpen} onOpenChange={setIsStatsModalOpen} />
    </div>;
};